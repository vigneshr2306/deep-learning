...
def readLanguages(Language1, Language2, vocab1=None, vocab2=None, reverse=False):
    print("Reading lines...")
    # Read the file and split into lines
    lines1 = open(Language1, encoding='utf-8').read().strip().split('\n')
    lines2 = open(Language2, encoding='utf-8').read().strip().split('\n')
    ...
    if reverse:
        ...
        input_Language = Language(Language2, vocab2)
        output_Language = Language(Language1, vocab1)
    else:
        input_Language = Language(Language1, vocab1)
        output_Language = Language(Language2, vocab2)
...
def prepareData(Language1, Language2, vocab1=None, vocab2=None, reverse=False, max_length=50):
    print('loading data from: ' + Language1)
    print('loading data from: ' + Language2)
    input_Language, output_Language, pairs = readLanguages(
        Language1, Language2, vocab1=vocab1, vocab2=vocab2, reverse=reverse)
    print("Read %s sentence pairs" % len(pairs))
    pairs = filterPairs(pairs, max_length)
    print("Trimmed to %s sentence pairs" % len(pairs))
    print("Counting words...")
    input_Language.loadVocab()
    output_Language.loadVocab()
    print("Counted words:")
    print("In vocabulary " + input_Language.vocab_path +
          ': ', input_Language.n_words)
    print("In vocabulary " + output_Language.vocab_path +
          ': ', output_Language.n_words)
    return input_Language, output_Language, pairs
...
def indexesFromSentence(Language, sentence):
    ...
    for word in sentence.split(' '):
        if word in Language.word2index:
            ...
            indexes.append(Language.word2index['<unk>'])
    return indexes
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion):
    ...
    encoder_outputs = torch.zeros(
        decoder.max_length, encoder.hidden_size, device=device)
        ...
        if iter > 1:
            if old_loss > loss:
                torch.save(encoder.state_dict(),
                           './model/encoder' + model_name + '.pt')
                torch.save(decoder.state_dict(),
                           './model/decoder' + model_name + '.pt')
...
def testBleuScore(encoder, decoder, pairs):
    ...
    for pair in pairs:
        try:
            ...
            pass
        output_sentence = ' '.join(output_words)
        bleu.append(sentence_bleu([output_sentence],
                    pair[1], smoothing_function=smooth))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'mode', type=str, help='Mode: train / test / translate')
    #parser.add_argument('--Language', type=str, default='V', help='{C: Czech, G: German,V: Vietnamese}')
    parser.add_argument('--gpu', type=str, default='0',
                        help='GPU index from 0, 1, 2')

    arg = parser.parse_args()

    device = torch.device(
        "cuda:" + arg.gpu if torch.cuda.is_available() else "cpu")
    ...
    input_Language, output_Language, pairs = prepareData(TRAIN_PATH[0], TRAIN_PATH[1], VOCAB_PATH[0], VOCAB_PATH[1], False,
                                                         max_length=MAX_LENGTH)

    hidden_size = 256
    encoder1 = EncoderRNN(input_Language.n_words, hidden_size).to(device)
    attn_decoder1 = AttnDecoderRNN(
        hidden_size, output_Language.n_words, dropout_p=0.2).to(device)
    if arg.mode == 'train':
        ...
        _, _, test_pair = prepareData(TEST_PATH[0], TEST_PATH[1], VOCAB_PATH[0], VOCAB_PATH[1], False,
                                      max_length=MAX_LENGTH)
        print("BLEU score:" + str(testBleuScore(encoder1, attn_decoder1, test_pair)))
